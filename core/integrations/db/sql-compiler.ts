...
function compileUpsertOperation(
  ...
  let conflictTarget = operation.conflictTarget ?? [
    ...getTablePrimaryKey(operation.table),
  ];
...
function compileReturningClause(returning: '*' | string[] | undefined): string {
  ...
  return (
    ' returning ' + returning.map((column) => quotePath(column)).join(', ')
  );
}
